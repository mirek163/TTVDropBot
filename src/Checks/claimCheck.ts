        ...
        if (autoclaim || preconditions) {
            ...
            for (const benefit of timedrop.benefitEdges) {
                if (timedrop.self.currentMinutesWatched === timedrop.requiredMinutesWatched && timedrop.self.dropInstanceID !== null) {
                    ...
                    if (autoclaim) winston.info(chalk.gray('Získáno ' + chalk.green(timedrop.name)), {event: "claim"})
                    if (preconditions && !autoclaim) winston.info(chalk.gray('Získáno ' + chalk.green(timedrop.name) + ' because otherwise cant watch next drop...'), {event: "claim"})
                }
            }
        }
    }
